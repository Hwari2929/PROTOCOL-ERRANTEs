...
## 10. 적용 시 결정 필요
1. **JetBrains Mono 번들 추가** (라벨/카탈로그) — 권장 예.
2. **색 토큰 전면 교체**(현 누런/어두운 톤 → BINDER 정밀값) + `main_theme.tres`.
3. **radius 0 → 3~8px**, **2.5px 보더 + 위계**, **톡톡 키섀도** 도입.
4. **덱 리플/팬/플립** 7장 사양으로 카드 모션 교체.
5. 전투 카드에 앞/뒷면(플립) 개념 도입 범위 — 드로우 시에만 짧은 플립 권장.
6. 재질 사물(클립/테이프/도장) 차용 범위(압정은 보유).
